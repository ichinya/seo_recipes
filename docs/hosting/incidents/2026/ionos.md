...
В [общей status-панели](https://status.ionos.cloud/) первое сообщение опубликовано **23 сентября в 17:15 UTC** (`Investigating`). В **18:32 UTC** статус сменился на `Identified`: провайдер сообщил об устранении высокой задержки базовой инфраструктуры хранения и продолжающемся поэтапном восстановлении управляющих экземпляров. На проверку **24 сентября** финального `Resolved` нет.

Ухудшена работа административных операций; существующие приложения, по сообщению IONOS, продолжают работать. Полный перечень регионов и клиентов не указан. Mitigation хранения не равен завершению восстановления управления. Событие не объединяется с августовским Kubernetes или S3-инцидентом без подтверждения общей причины.

[Карточка события](https://status.ionos.cloud/incidents/9d0yl9gsjtm7) сохранена по ссылке панели, но отдельно не загрузилась. Время жизни записи не выдаётся за непрерывный простой. Проверяйте API управления отдельно от внешнего health-check приложения, а после timeout выясняйте результат операции перед повтором. Категория **«Рискованные»** сохраняется; собственных эксплуатационных тестов здесь нет.

## Подтверждённые события

| Дата | Сервис | Что произошло | Воздействие | Статус / источник |
| --- | --- | --- | --- | --- |
| с 23 сентября | Managed Kubernetes | Нестабильность control plane; применена мера против высокой storage latency | Административные операции деградировали, workloads заявлены работающими | На проверку 24 сентября `Identified`; [общая панель](https://status.ionos.cloud/) · [карточка](https://status.ionos.cloud/incidents/9d0yl9gsjtm7) |
