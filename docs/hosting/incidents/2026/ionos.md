...
- [DBaaS migrations и дедлайны 2026](../../info/ionos-dbaas-migrations-2026.md)
...
TXL переведён в `Resolved` **21 сентября в 07:24 UTC**, после восстановления сервисов ещё 9 сентября. В **08:31 UTC 21 сентября** объявлено расширение GPU capacity и восстановление обычной доступности provisioning GPU Server. Для закрытого S3-инцидента ранее опубликован RCA; ограничения Cloud Support закрыты. Подробные временные рамки и границы воздействия приведены ниже.

## 26 сентября — Data Center Designer

[Карточка DCD](https://status.ionos.cloud/incidents/fpdt04ng3rvm) описывает бесконечную загрузку интерфейса с **00:25 UTC**. Доступность объявлена восстановленной в **02:57**, в **03:04** продолжалось наблюдение. [Общая панель](https://status.ionos.cloud/) уже содержит финальный `Resolved` **26 сентября в 11:11 UTC**; отдельное представление карточки при проверке ещё показывало monitoring.
...
[Отдельная карточка](https://status.ionos.cloud/incidents/3qfh1qnp0wmj) и [общая панель](https://status.ionos.cloud/) фиксируют сообщение о сетевой деградации **08:43 UTC**, дополнение о задержках создания ресурсов **08:58**, установку исправления / `Monitoring` **11:57** и `Resolved` **14:33**. Финальные этапы прочитаны на общей панели; отдельная карточка при проверке содержала более ранние сообщения.
...
Первое сообщение — **23 сентября в 17:15 UTC** (`Investigating`), в **18:32** — `Identified` после меры против высокой storage latency. При первоначальной проверке 24 сентября финал ещё не был прочитан. Теперь [карточка события](https://status.ionos.cloud/incidents/9d0yl9gsjtm7) подтверждает mitigation приблизительно **23 сентября в 19:00 UTC** и публикацию `Resolved` **24 сентября в 05:43 UTC**. Проверено **26 сентября**.
...
| --- | --- | --- | --- | --- |
| 26 сентября | Data Center Designer | Интерфейс застревал на загрузке | Управление через DCD, не подтверждённая остановка VM | Доступен с 02:57, `Resolved` 11:11 UTC по [общей панели](https://status.ionos.cloud/); [карточка](https://status.ionos.cloud/incidents/fpdt04ng3rvm) |
| 24 сентября | Сеть TXL / provisioning | Потери пакетов и задержки создания ресурсов | Сетевое качество отдельных ресурсов и управление оцениваются раздельно | Fix / `Monitoring` 11:57, `Resolved` 14:33 UTC; [карточка](https://status.ionos.cloud/incidents/3qfh1qnp0wmj) · [финал на общей панели](https://status.ionos.cloud/) |
| 23–24 сентября | Managed Kubernetes, DE/FRA | Нестабильность control plane; применена мера против высокой storage latency | Административные операции деградировали, workloads заявлены работающими | Mitigation около 23 сентября 19:00, `Resolved` 24 сентября 05:43 UTC; [карточка](https://status.ionos.cloud/incidents/9d0yl9gsjtm7) |
| 8–9 сентября; закрытие 21 сентября | Сеть DE/TXL | Сетевой сбой; команда связала проблему с недавней заменой маршрутизатора | Compute, Storage, Network, Provisioning и DBaaS восстанавливались поэтапно | `Resolved` 21 сентября, 07:24 UTC по [общей панели](https://status.ionos.cloud/); [карточка TXL](https://status.ionos.cloud/incidents/2b5lgdhvt047). Monitoring после 9 сентября не равен непрерывному outage |
...
| 18 июня — 21 сентября | GPU Server | Из-за дефицита ёмкости создание нового GPU-сервера или повторный запуск остановленного мог завершаться ошибкой | Рекомендация не выключать работающий GPU относилась к периоду ограничения | `Resolved` 21 сентября, 08:31 UTC: capacity расширена; [общая панель](https://status.ionos.cloud/) · [карточка](https://status.ionos.cloud/incidents/0dv04n68ysxq) |
...
Хронология по [официальной карточке](https://status.ionos.cloud/incidents/2b5lgdhvt047); время — UTC. Финальное закрытие добавлено по [общей панели](https://status.ionos.cloud/) при проверке 22 сентября:
...
Для GPU IONOS рекомендовал не выключать работающий server **во время ограничения**. По [обновлению общей панели](https://status.ionos.cloud/) от **21 сентября, 08:31 UTC** дополнительная ёмкость введена, ограничение закрыто. Это не гарантия свободного GPU любой конфигурации в будущем и не результат нашего теста. Отдельное ограничение MongoDB `de/fra/2` этим сообщением не закрывается.
...
- [IONOS DBaaS — миграции API и дедлайны версий](../../info/ionos-dbaas-migrations-2026.md)
...
- [IONOS DBaaS migrations и дедлайны 2026](../../info/ionos-dbaas-migrations-2026.md)
- [Методика журнала инцидентов](./coverage.md)

## Источники

- [IONOS Cloud Status: финальные обновления TXL и GPU 21 сентября](https://status.ionos.cloud/)
- [GPU capacity: карточка события](https://status.ionos.cloud/incidents/0dv04n68ysxq)
