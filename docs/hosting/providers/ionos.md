...
Операционные статусы и DBaaS-дедлайны ниже проверены **7 сентября 2026 года**; старые примеры тарифов не являются новым ценовым замером.
...
- 26 августа — 3 сентября: повышенную задержку чтения и записи S3 Object Storage в `eu-central-1`; событие закрыто;
...
- с 4 сентября — ограниченную доступность телефонной поддержки Cloud Support; обновление 7 сентября предупреждает и о задержках ответов поддержки.
...
## S3 Object Storage: инцидент 26 августа — 3 сентября закрыт

3 сентября в **13:39 UTC** IONOS отметил восстановление нормального времени ответа S3 в `eu-central-1`. История события и ограничения интерпретации длительности находятся в [журнале инцидентов](../incidents/2026/ionos.md).
...
- в августе–сентябре зафиксированы как сбой Object Storage management, так и длительная деградация S3 read/write latency; оба события закрыты;
...
- открытое ограничение Cloud Support может увеличить время получения помощи.
...
Категория остаётся **«Рискованные»**. Закрытие S3-инцидента улучшает текущий операционный статус, но не отменяет оснований, связанных с аккаунтом, оплатой и санкционной политикой.
