...
- **Почтовые ограничения.** Поддержка сообщила, что исходящий TCP/25 не блокируется и PTR доступен по запросу. Перед переносом почты нужно проверить соединение с внешними MX, обратную запись, репутацию IPv4 и доставляемость в основные почтовые сервисы.
