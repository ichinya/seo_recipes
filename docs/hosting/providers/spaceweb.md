---
title: SpaceWeb / Sweb
description: Исторический личный опыт SpaceWeb, свежий тест VDS, домены и DNS
...
У SpaceWeb есть акция с бесплатным тестовым периодом на VPS и S3 до 31 января 2027 года. Для участия нужно выбрать тариф, зарегистрироваться, подтвердить номер телефона и активировать тест в панели. По наблюдению в панели, тестовый VDS можно попробовать только до первой оплаты аккаунта и после подтверждения номера телефона.
...
## Баланс и приостановка

В оферте SpaceWeb от 1 июля 2026 года услуги описаны как предоплатные. Для услуг с использованием платформы списания идут ежедневно, а если услуга тарифицируется в рублях за час, то ежечасно. Если на лицевом счете платформы не хватает денег для оплаты полного дня или часа, списывается имеющийся остаток за этот день или час, а оказание услуги приостанавливается в течение суток или часа.

Для VDS важна отдельная формулировка: отсутствие предоплаты более 14 календарных дней по тарифам `VDS` признается отказом клиента от дальнейшего использования услуг. На 15-й день договор по услугам с использованием платформы считается расторгнутым, а файлы, сведения и данные могут быть удалены с серверов SpaceWeb без дополнительного предупреждения.

Слово "отрицательный баланс" в оферте прямо используется для услуги размещения оборудования: при задолженности больше 3 календарных дней провайдер вправе отключить оборудование. Для VDS в найденных формулировках важнее не сам "минус", а нехватка предоплаты на лицевом счете платформы. При образовании задолженности по оплате услуг SpaceWeb также вправе взыскать неустойку 0,1% от суммы задолженности за каждый день просрочки, но только на основании письменного требования.

7 июля 2026 года после тестового VDS в панели дополнительно появился отрицательный баланс по разделу `Облако`: сначала `-5.28 ₽`, позже чуть больше `-9 ₽`. В ответе по заявке поддержка SpaceWeb написала, что минус образуется по техническим причинам и его исправят в ближайшее время. После закрытия тестового VDS баланс снова стал нулевым. Практически это означает, что неожидательный минус после теста лучше сразу фиксировать скриншотом и уточнять через поддержку, а не считать его нормальной платой за тестовый период. При обычном использовании несколько рублей легко не заметить, особенно если по облачным услугам идут ежедневные или ежечасные списания.

В правилах также указано, что при прекращении или приостановлении оказания услуг срок хранения веб-системы для `VDS` составляет 14 календарных дней, после чего веб-система и резервные копии удаляются, а доступ к панели блокируется. Практический вывод: для VDS нельзя рассчитывать на долгий grace period после ухода баланса в минус или остановки оплаты; автоплатежи и внешние бэкапы обязательны.

## Домены и DNS

У SpaceWeb есть регистрация и управление доменами, но по свежему личному опыту этот раздел требует отдельной проверки после каждой операции.

При смене DNS-серверов для домена SpaceWeb отправляет письмо на email администратора домена. В письме указано, что для смены DNS нужно подтвердить операцию по ссылке, а ссылка действует 24 часа. В проверенном кейсе запрашивалась смена на стандартные DNS-серверы SpaceWeb: `ns1.spaceweb.ru`, `ns2.spaceweb.pro`, `ns3.spaceweb.pro`, `ns4.spaceweb.pro`. Практический вывод: смена DNS не применяется просто по нажатию в панели, нужно иметь доступ к административной почте домена и не пропустить письмо подтверждения.

По покупке домена появился более неприятный сигнал. После оплаты пришло письмо SpaceWeb об успешной регистрации домена: в нем указано, что регистратором является Руцентр, управление и продление доступны через SpaceWeb в разделе "Мои домены", а домен начнет работать и будет доступен в интернете в течение суток. Но примерно через 20 минут после покупки в панели "Мои домены" тот же домен еще отображался как свободный, с предложением зарегистрировать его за отдельную плату. Управление доменом и смена DNS в этот момент тоже фактически не работали: при переходе по ссылке подтверждения смены DNS появлялась ошибка отправки запроса и рекомендация связаться с поддержкой. Сам домен, номер заявки и данные аккаунта в статье не публикуются.

Обращение в поддержку ушло сразу после обнаружения проблемы. Заявка была создана почти мгновенно, AI-помощник SpaceWeb сразу не смог подобрать решение в базе знаний и передал обращение в поддержку. Примерно через 3-4 часа после покупки домен стал отображаться в панели корректно; до этого панель показывала, что домен свободен. Живой ответ поддержки пришел примерно через 3 часа 45 минут после обращения: специалист написал, что регистрация домена занимает некоторое время, и к моменту ответа статус домена уже успешно обновился в панели.

Практический вывод: после покупки домена у SpaceWeb нужно сохранять письмо об успешной регистрации, проверять списание денег, статус в панели и при расхождении сразу писать в поддержку. Если через 15-20 минут после покупки домен в панели еще показывается свободным, это может быть задержкой синхронизации; в проверенном случае статус стал корректным примерно через 3-4 часа (но может идти до 24 часов). До обновления статуса в панели такой домен лучше не считать полностью оформленным с точки зрения управления через SpaceWeb.
...
- есть 3-дневный тестовый период, но тестовый VDS доступен только до первой оплаты аккаунта и после подтверждения телефона;
...
- после теста в панели появился отрицательный баланс по `Облаку`, который дошел до суммы чуть больше `-9 ₽`; поддержка объяснила это техническими причинами, а после закрытия тестового VDS баланс снова стал нулевым;
- смена DNS-серверов домена требует подтверждения через email администратора домена, ссылка подтверждения действует 24 часа;
- примерно через 20 минут после оплаты и письма об успешной регистрации домена панель "Мои домены" все равно показывала домен как свободный и предлагала зарегистрировать его заново; статус стал корректным примерно через 3-4 часа после покупки;
- при проблеме с доменом AI-помощник сразу передал заявку в поддержку, но живой ответ пришел примерно через 3 часа 45 минут после обращения;
- при недостатке средств услугу могут приостановить, задолженность может повлечь неустойку, а для `VDS` отсутствие предоплаты более 14 дней по оферте ведет к риску удаления данных без дополнительного предупреждения;
...
- [Оферта SpaceWeb от 01.07.2026](https://sweb.ru/docs/oferta.pdf)
