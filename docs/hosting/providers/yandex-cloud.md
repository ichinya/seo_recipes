---
title: Yandex Cloud
description: Заметка по Yandex Cloud как отдельному российскому облачному провайдеру с учетом инцидентов и продуктовых изменений 2026 года
icon: fa-solid fa-circle-check
category: Хостинг
tag: [Хостинг, Облако, Yandex Cloud, Яндекс, 152-ФЗ, УЗ-1, Инциденты, Workflows, AI Studio, EventRouter]
---
...
## Serverless Integrations: закрытие и перенос Workflows

Выборочная проверка **11 сентября 2026 года**: [актуальное уведомление](https://yandex.cloud/ru/docs/serverless-integrations/sunset) уточняет дату перехода управления Workflows в AI Studio — **4 сентября**, а не 3 сентября из ранее использованной документации. Сами Workflows сохраняются; API Gateway продолжает работать отдельно.

EventRouter становится read-only **15 сентября**, а Serverless Integrations прекращает работу **8 октября**. Это уже не только смена интерфейса: необходимо проверить зависимые источники событий, получателей и автоматизацию.

Календарь, условия получения резервных копий и безопасный план переключения собраны в [инструкции по миграции EventRouter](../info/yandex-serverless-integrations-sunset-2026.md). В частности, нельзя считать совместное чтение одной Message Queue старым и новым контуром корректным сравнительным тестом.

Категория **«Рекомендую»** сохраняется. Прекращение сервиса учитывается как риск жизненного цикла продукта, а не как подтверждённая авария всего облака. До переноса нужно проверить права команды в AI Studio, запуск Workflows и обработку событий приложением.
...
- изменения интерфейсов и прекращение managed-сервисов требуют миграционного плана, а не только обновления runbooks.
...
- сохранение поведения приложения после замены EventRouter, включая дубли, фильтры и обработку ошибок.
...
Перенос Workflows и закрытие EventRouter не меняют категорию автоматически, но требуют проверки приложения. Актуальные сроки и порядок подготовки — в [миграционной инструкции](../info/yandex-serverless-integrations-sunset-2026.md).
...
- [Закрытие Serverless Integrations: актуальные сроки](https://yandex.cloud/ru/docs/serverless-integrations/sunset)
- [Миграция EventRouter на триггеры](https://yandex.cloud/ru/docs/serverless-integrations/tutorials/eventrouter-migration)
- [Yandex Workflows: описание и уведомление о миграции UI](https://yandex.cloud/ru/docs/serverless-integrations/concepts/workflows/workflow)
- [Конструктор шагов Yandex Workflows](https://yandex.cloud/ru/docs/serverless-integrations/operations/workflows/constructor/)
